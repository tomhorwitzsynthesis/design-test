...
# ---------------- Config ----------------
# Target month for updates (set to None to use previous month automatically)
TARGET_YEAR = 2025
...
UPDATE_AGILITY_PREFIX = "Agility_"  # for monthly update files like Agility_darnu.xlsx
...
def expected_update_csv_candidates(brand_raw: str, month_str: str) -> List[str]:
    ...
    Plausible update filenames for the brand and month (case-insensitive match used later):
      'Agility_Brand.xlsx' (new format)
      'Brand - Month.csv'
      'Brand_Month.csv'
      'Brand.csv' (fallback)
    """
    return [
        f"{UPDATE_AGILITY_PREFIX}{brand_raw}.xlsx",
...
def read_csv_with_normalized_cols(path: Path) -> Tuple[pd.DataFrame, Dict[str, str]]:
    """Read a CSV or Excel file and return DataFrame with normalized column names."""
    # Determine file type and read accordingly
    if path.suffix.lower() in ['.xlsx', '.xls']:
        # For Excel files, try to read the first sheet or 'Raw Data' sheet
        try:
...
def restore_original_order_cols(base_df: pd.DataFrame, cols_order_source: pd.DataFrame) -> List[str]:
    ...
    Only includes columns that actually exist in base_df.
    """
    source_cols = [c for c in cols_order_source.columns if c in base_df.columns]
...
def update_main_csv_files(target_year: int, target_month: int) -> None:
    ...
    for brand in sorted(all_brand_slugs):
        ...
        if not update_path:
            logger.info("No update file found for brand '%s' for %s; leaving main file unchanged.", brand, month_str)
            continue

        try:
            logger.info("Processing main data: %s <- %s", main_path.name, update_path.name)
    ...
    for brand, (compos_path, agility_path) in sorted(mapping.items()):
        ...
        try:
            ...
            if agility_norm.shape[1] > 0:
                ...
                restored_order = restore_original_order_cols(combined, agility_norm)
                combined = combined[restored_order]
...
def main(target_year: Optional[int] = None, target_month: Optional[int] = None):
    """
    Run both update processes for the target month (default: uses TARGET_YEAR/TARGET_MONTH config or previous month).
    """
    # Use provided arguments, then config variables, then fall back to previous month
